...
export const getCategoryWithProducts = async (req, res, next) => {
  try {
    ...
    const queryFilter = {
      ...
      [`${filterField}.Назва товару`]: { $exists: true, $ne: '' },
    };

    if (minPrice || maxPrice) {
      queryFilter[`${filterField}.RetailPrice`] = {};
      if (minPrice) queryFilter[`${filterField}.RetailPrice`]['$gte'] = Number(minPrice);
      if (maxPrice) queryFilter[`${filterField}.RetailPrice`]['$lte'] = Number(maxPrice);
    }
    ...
    const updatedProducts = products.map(product => ({
      ...product.toObject(),
      photos: generatePhotoUrls(product.photos || []),
    }));
...
export const getProductsForSubcategory = async (req, res, next) => {
  try {
    ...
    if (!subcategory) {
      throw HttpError(404, 'Подкатегория не найдена');
    }
...
export const getProductByCategoryOrSubcategory = async (req, res, next) => {
  try {
    ...
    if (!subcategory) {
      throw HttpError(404, 'Підкатегорія не знайдена');
    }

    const product = await Product.findOne({ offerId: productId });
    if (!product) {
      throw HttpError(404, 'Продукт не знайден');
    }
...
const generatePhotoUrls = photos => {
  if (!photos || photos.length === 0) {
    return [];
  }
  return photos.map(photo => `${process.env.PHOTOS_URL}/${photo}`);
};
